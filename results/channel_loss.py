...
for d in dists:
    r0_d += [r0(cn2_day, d, wl)]
    r0_n += [r0(cn2_night, d, wl)]
...
fov_d = 4*np.pi*np.sin(ws_d/2)**2
fov_n = 4*np.pi*np.sin(ws_n/2)**2



# plt.figure()
# plt.plot(dists, ws_d, linewidth=2, label='Day')
# plt.plot(dists, ws_n, linewidth=2, label='Night')

# # plt.plot(dists, counts_d, linewidth=2, label='Day')
# # plt.plot(dists, counts_n, linewidth=2, label='Night')

# plt.xlabel('Distance (m)',fontsize=13)
# plt.ylabel('Angle of arrival', fontsize=13)
...
counts_d = 2.4*0.004*fov*np.pi*(.075**2)*wl/(sp.constants.c* sp.constants.h)*10**(-det_loss/10)
...
background_night = 2.4*0.00008*fov*np.pi*(.075**2)*wl/(spc.c* spc.h)*10**(-det_loss/10) 

print('Background day: ', background_day)
print('Background night: ', background_night)


# rad_night = (sp.constants.c* sp.constants.h)*background_night/(fov*np.pi*(.075**2)*wl*10**(-det_loss/10))
# print('Rad night: ', rad_night)
...
fig = plt.figure()
plt.plot(dists, sigs_d*1e12, 'o', label='Day', linewidth=3)
plt.plot(dists, sigs_n*1e12, 'o', label='Night', linewidth=3)
# plt.xlabel('Distance (seg)', fontsize=15)
...
fig = plt.figure()
plt.plot(dists, coincidences_d, 'o', label='Day', linewidth=3)
plt.plot(dists, coincidences_n, 'o', label='Night', linewidth=3)
# plt.xlabel('Distance (seg)', fontsize=15)
plt.ylabel('Precision (picoseg)', fontsize=15)
plt.xticks(fontsize=13, rotation=0)
plt.yticks(fontsize=13, rotation=0)
